...
T* copy_array(const T *src, int size)                       //THIS IS BASICALLY THE SAME AS REALLOCATE
{
    ...
    T* start_ptr = new_array;                               //Point to the start of our new array
    T* end_ptr = start_ptr + size;                          //End pointer just adds to size

    for( start_ptr; start_ptr != end_ptr ; start_ptr++)
    {
        *start_ptr = *src;                                  //Dereference values to copy
        src++;                                              //Increment source since start_ptr increments in loop
    }
